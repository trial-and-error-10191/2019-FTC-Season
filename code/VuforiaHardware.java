...
public class VuforiaHardware {
    ...
    // Vector and Orientation Variables
    public int hIndex = 0; // Horizontal Dimension of the image
    public int dIndex = 2; // Depth dimension of the image
    ...
    boolean useWebcam; // whicih camera phone? or webcam?
    ...
    final float CAMERA_LEFT_DISPLACEMENT = -6.5f; // its from end now
    final float CAMERA_FROM_EDGE = -13.5f;
    ...
    public double getCameraOffsetFromSkystone() {
    ...
    public double getDistanceToSkystone() {
        VectorF translation = currentSkystoneLocation.getTranslation(); // Acquire vector to center of skystone
        return translation.get(dIndex)/25.4; // Return depth distance in inches
    }
    ...
    public void detectSkystone(Telemetry telemetry) {
        setCamera("webcam");
        if (((VuforiaTrackableDefaultListener) stoneTarget.getListener()).isVisible()) {
            skyStoneFlag = true;
            currentSkystoneLocation = ((VuforiaTrackableDefaultListener) stoneTarget.getListener()).getFtcCameraFromTarget();
        }
    }
